...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    node distance=0.2cm,
    buf/.style={draw, minimum width=10cm, minimum height=0.9cm, align=left},
    >=Stealth
  ]
    \node[buf, fill=gray!10] (b1) {Buffer 1: [ ... data ... ][ \# sentinel ]};
    \node[buf, fill=gray!06, below=of b1] (b2) {Buffer 2: [ ... data ... ][ \# sentinel ]};
    \draw[->] (b1.east) -- ++(0.6,0) node[right]{isi ulang saat sentinel};
    \draw[->] (b2.east) -- ++(0.6,0) node[right]{isi ulang saat sentinel};
  \end{tikzpicture}
