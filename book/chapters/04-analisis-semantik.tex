...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    node distance=1.4cm,
    scope/.style={rectangle, draw, rounded corners, align=left, minimum width=4.1cm, minimum height=0.9cm},
    >=Stealth
  ]
    \node[scope, fill=gray!10] (g) {Global: \{ f: fun, x: int \}};
    \node[scope, fill=gray!08, below=of g] (s1) {Scope f: \{ a: int, y: bool \}};
    \node[scope, fill=gray!06, below=of s1] (s2) {Blok: \{ x: string \}};
    \draw[->] (s2.west) -- ++(-0.6,0) |- (s1.west);
    \draw[->] (s1.west) -- ++(-0.6,0) |- (g.west);
  \end{tikzpicture}
